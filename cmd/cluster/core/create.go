...
func bindCoreOptions(opts *RawCreateOptions, flags *pflag.FlagSet) {
	...
	flags.StringSliceVar(&opts.DisableClusterCapabilities, "disable-cluster-capabilities", nil, "Optional cluster capabilities to disabled. The only currently supported values are ImageRegistry,Console,openshift-samples.")
